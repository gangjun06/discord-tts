...
func (v *Voice) SetVoiceConnection(vc *discordgo.VoiceConnection) {
	v.Vc = vc
}

// SendPCM will receive on the provied channel encode
// received PCM data into Opus then send that to Discordgo
func (v *Voice) SendPCM(pcm <-chan []int16) {
	if pcm == nil {
		return
	}
	...
	v.OpusEncoder, err = gopus.NewEncoder(frameRate, channels, gopus.Audio)
	...
	for {
		...
		opus, err := v.OpusEncoder.Encode(recv, frameSize, maxBytes)
		if err != nil {
			OnError("Encoding Error", err)
			return
		}

		if !v.Vc.Ready || v.Vc.OpusSend == nil {
...
func (v *Voice) ReceivePCM(c chan *discordgo.Packet) {
	if c == nil {
		return
	}
	...
	for {
		if !v.Vc.Ready || v.Vc.OpusRecv == nil {
			OnError(fmt.Sprintf("Discordgo not to receive opus packets. %+v : %+v", v.Vc.Ready, v.Vc.OpusSend), nil)
			return
		}
		...
		if v.Speakers == nil {
			v.Speakers = make(map[uint32]*gopus.Decoder)
		}

		_, ok = v.Speakers[p.SSRC]
		if !ok {
			v.Speakers[p.SSRC], err = gopus.NewDecoder(48000, 2)
			if err != nil {
		...
		p.PCM, err = v.Speakers[p.SSRC].Decode(p.Opus, 960, false)
		if err != nil {
...
func (v *Voice) PlayAudioFile(filename string, stop <-chan bool) {
	if v.Vc == nil {
		return
	}
